...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("Acc_type" == $this->xml_array['tables'][0])
			$isDB = true;
		if("asst_desc" == $this->xml_array['tables'][0])
			$isDB = true;
		if("BRI" == $this->xml_array['tables'][0])
			$isDB = true;
		if("main" == $this->xml_array['tables'][0])
			$isDB = true;
		if("FM_Income" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Org_info" == $this->xml_array['tables'][0])
			$isDB = true;
		if("PFLI" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Produ_Info" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Remitance" == $this->xml_array['tables'][0])
			$isDB = true;
		if("St_Info" == $this->xml_array['tables'][0])
			$isDB = true;
		if("asset_des" == $this->xml_array['tables'][0])
			$isDB = true;
		if("inv-risk" == $this->xml_array['tables'][0])
			$isDB = true;
		if("R-1" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Approv" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Req" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Month_sales" == $this->xml_array['tables'][0])
			$isDB = true;
		if("userinfo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("main" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
